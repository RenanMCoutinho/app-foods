import{a as g,i as M,j as R,q as F,h as N,e as T,w as b,k as h}from"./firebase-DXeuB9Ti.js";/* empty css              */import{i as B}from"./auth-guard-ahADkYB_.js";let s,e;async function G(){const a=g.currentUser;if(!a)return;const r=document.querySelector("#welcome-message");r&&(r.textContent=`Olá, ${window.currentUserData?.nome||a.email.split("@")[0]}!`),document.querySelector(".logout-btn")?.addEventListener("click",async()=>{confirm("Deseja sair?")&&(await M(g),window.location.href="/app-foods/pages/login.html")});const i=new Date;s=i.getFullYear(),e=i.getMonth(),document.querySelector("#btn-mes-anterior")?.addEventListener("click",()=>{e===0?(e=11,s--):e--,f(a.uid)}),document.querySelector("#btn-mes-proximo")?.addEventListener("click",()=>{e===11?(e=0,s++):e++,f(a.uid)}),f(a.uid)}async function f(a){const r=document.querySelector("#mes-label"),i=new Date(s,e,1);r&&(r.textContent=i.toLocaleDateString("pt-BR",{month:"long",year:"numeric"}));const y=`${s}-${String(e+1).padStart(2,"0")}-01`,w=new Date(s,e+1,0).getDate(),S=`${s}-${String(e+1).padStart(2,"0")}-${String(w).padStart(2,"0")}`,c=document.querySelector("#kpi-total"),d=document.querySelector("#kpi-ganhos"),l=document.querySelector("#kpi-dias"),u=document.querySelector("#historico-mes");c&&(c.textContent="..."),d&&(d.textContent="..."),l&&(l.textContent="...");try{const o=(await R(F(N(T,"entregas"),b("motoristaId","==",a),b("data",">=",y),b("data","<=",S),h("data","desc"),h("createdAt","desc")))).docs.map(t=>({id:t.id,...t.data()})),k=o.reduce((t,p)=>t+(p.valor||0),0),v=new Set(o.map(t=>t.data)).size;if(c&&(c.textContent=o.length),d&&(d.textContent=`R$ ${k.toFixed(2).replace(".",",")}`),l&&(l.textContent=v),!o.length){u.innerHTML=`<div class="p-8 text-center">
