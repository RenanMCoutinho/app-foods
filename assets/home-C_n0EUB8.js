import{l as y,d as h,e as i,a as c,i as b,q as f,h as d,w as l,j as v,k as w,g as E,f as S}from"./firebase-DXeuB9Ti.js";/* empty css              */import{i as $}from"./auth-guard-ahADkYB_.js";let p=[],n=[];async function j(){const t=c.currentUser;if(!t)return;const r=new Date,a=document.querySelector("#data-hoje");a&&(a.textContent=r.toLocaleDateString("pt-BR",{weekday:"long",day:"2-digit",month:"long"})),document.querySelector(".logout-btn")?.addEventListener("click",async()=>{confirm("Deseja sair?")&&(await b(c),window.location.href="/app-foods/pages/login.html")});const s=window.currentUserData?.supervisorId;await q(s),await T(t.uid),document.querySelector("#select-empresa")?.addEventListener("change",e=>{const o=e.target.value;L(o)})}async function q(t){const r=document.querySelector("#select-empresa");try{const a=t?f(d(i,"empresas"),l("supervisorId","==",t)):d(i,"empresas");p=(await v(a)).docs.map(e=>({id:e.id,...e.data()})),p.forEach(e=>{r.innerHTML+=`<option value="${e.id}">${e.nome}</option>`})}catch(a){console.error("Erro ao carregar empresas",a)}}function L(t){const r=document.querySelector("#card-tipos"),a=document.querySelector("#lista-tipos");if(!t){r.classList.add("hidden");return}const s=p.find(e=>e.id===t);if(!s||!s.tipos?.length){a.innerHTML='<p class="text-sm text-slate-400 italic">Esta empresa não tem tipos de entrega cadastrados.</p>',r.classList.remove("hidden");return}a.innerHTML=s.tipos.map((e,o)=>`
    ...
    `).join("")}window.removerEntrega=async t=>{confirm("Remover esta entrega do dia?")&&(await y(h(i,"entregas",t)),n=n.filter(r=>r.id!==t),m())};$(["motorista"]);window.addEventListener("load",()=>{j()});
